...
const Phone = () => {
  ...
  if (isLoading) return <p className="p-4 text-center mt-30">Loading...</p>;

  return (
    <>
      <div className="bg-blue-600 w-full">
        <header className="container mx-auto flex justify-between items-center  text-white px-6 py-4">
          <h1 className="text-2xl font-semibold">Phone Store</h1>
          <button
            onClick={() => {
              setEditingItem(null);
              setIsModalOpen(true);
            ...
            {phone.imageUrl && (
              <div className="mb-4">
                <img
                  src={phone.imageUrl}
                  alt={phone.title}
                  className="size-50 mx-auto rounded-md border-1 border-gray-300"
                />
              </div>
            )}
